...
const UserEventsWaterfalls: FC<UserEventsProps> = function (props) {
  const { orgUser, changeBrowserMode } = props;
  ...
  const renderContent = () => {
    ...
    return (
      <EventTimeLine
        data={data.source}
        formatHref={(eventId) =>
          `/me/crm/users/${orgUser.organizationUserId}/events/${eventId}`
        }
      />
    );
  };
...
export default UserEventsWaterfalls;
