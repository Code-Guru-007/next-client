...
const OrgMediaFieldSlider: FC<OrgMediaFieldProps> = function (props) {
  ...
  const sliderMediaList = watch("organizationMediaSliderMediaList");
  ...
  // } = useDragItem("", sliderMediaList);
  // console.log("sliderMediaList", sliderItemsRef.current);

  const slidersSelectedSize = useMemo(
    () =>
      selectedDesktop
        ? sliderMediaList?.filter(
            (el) =>
    ...
    [selectedDesktop, sliderMediaList]
  );
...
export default OrgMediaFieldSlider;
