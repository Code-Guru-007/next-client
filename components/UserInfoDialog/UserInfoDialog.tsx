...
import { Box, Button, CircularProgress } from "@eGroupAI/material";
...
export const DIALOG = "UserInfoDialog";
...
const UserInfoDialog: FC<UserInfoDialogProps> = function (props) {
  const { openSnackbar } = useReduxSnackbar<SnackbarProps>(SNACKBAR);

  const { onConfirm, defaultValues, loading, orgUser } = props;
  ...
  const getUpdatePayload = useUpdateUserApiPayload(
    orgUser?.dynamicColumnTargetList,
    orgColumns?.source
  );
  ...
  const handleCloseWithConfirmLeave = useConfirmLeaveDialog({
  ...
  return (
    ...
    >
      ...
      <DialogFullPageContainer id="column-dialog">
        <form onSubmit={onSubmit} style={{ position: "relative" }}>
          ...
          <DialogContent style={{ overflow: "unset" }}>
            <Box
              sx={{ mt: 1 }}
              className={clsx(preview && classes.hiddenContainer)}
            >
              <Card sx={{ width: "100%" }}>
                <CardHeader title="" />
                    ...
                    if (el.sortKey && el.columnType) {
                      return (
                        <Stack spacing={1.5} key={el.id}>
                          ...
                          <DynamicField
                            capitalize={
                              el.sortKey === "organizationUserIdCardNumber"
                            }
            ...
            <UserInfoPreview previewValue={previewValue} isPreview={preview} />
          </DialogContent>
      ...
      <DialogActions>
        <DialogCloseButton onClick={() => handleCloseWithConfirmLeave()} />
...
export default React.memo(UserInfoDialog);
