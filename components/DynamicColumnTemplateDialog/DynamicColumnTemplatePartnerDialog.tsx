...
import DynamicColumnTemplatePartnerForm from "./DynamicColumnTemplatePartnerForm";

export const DIALOG = "DynamicColumnTemplatePartnerDialog";
...
const DynamicColumnTemplatePartnerDialog: FC<DynamicColumnTemplateDialogProps> =
  function (props) {
    ...
    const methods = useForm<DynamicColumnTemplatePartnerFormInput>({
      defaultValues: {
    ...
    const {
      excute: createOrgDynamicColumnTemplatePartner,
      isLoading: isCreating,
    } = useAxiosApiWrapper(
      apis.org.createOrgDynamicColumnTemplatePartner,
      "Create"
    );

    const {
      excute: updateOrgDynamicColumnTemplatePartner,
      isLoading: isUpdating,
    } = useAxiosApiWrapper(
      apis.org.updateOrgDynamicColumnTemplatePartner,
      "Update"
    );
    ...
    const handleNext = async () => {
      ...
      if (activeStep === 1) {
        ...
        try {
          if (templateToUpdate) {
            await updateOrgDynamicColumnTemplatePartner({
              organizationColumnTemplateId:
            ...
            await createOrgDynamicColumnTemplatePartner({
              organizationId,
              organizationColumnTemplateTitle:
                values.organizationColumnTemplateTitle,
              organizationColumnList: values.organizationColumnList,
      ...
      >
        ...
        <DialogFullPageContainer>
          <FormProvider {...methods}>
            {!isLoadingData && (
              <DynamicColumnTemplatePartnerForm
...
export default DynamicColumnTemplatePartnerDialog;
