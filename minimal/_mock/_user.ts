...
export const _userCards = [...Array(21)].map((_, index) => ({
  ...
  avatarUrl: _mock.image.avatar(index) || "",
  totalFollowers: _mock.number.nativeL(index) || 8,
  totalPosts: _mock.number.nativeL(index + 2) || 0,
  totalFollowing: _mock.number.nativeL(index + 1) || 0,
}));
...
export const _userList = [...Array(20)].map((_, index) => ({
  ...
  name: _mock.fullName(index) || "",
  isVerified: _mock.boolean(index) || false,
