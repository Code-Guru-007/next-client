...
import QuestionEditable, {
  OptionType,
  QuestionEditableProps,
} from "./QuestionEditable";
import {
  Question as QuestionTyping,
  QuestionType,
  Option,
} from "../Survey/types";
...
const SurveyQuestions: FC<SurveyQuestionsProps> = ({
  ...
  const { questions, setQuestions, selectedQuestionId, setSelectedQuestionId } =
  ...
  const handleNameChange: QuestionEditableProps["onNameChange"] = (
    event,
    question
  ) => {
  ...
  const handleTypeChange: QuestionEditableProps["onTypeChange"] = (
    event,
    question
  ) => {
    ...
    if (onTypeChange) {
      onTypeChange(
        ...
        question,
        nextQuestion,
        shouldCreateOneOption ? createdOption : undefined
      );
    }
  };

  const handleDescChange: QuestionEditableProps["onDescChange"] = (
    event,
    question
  ) => {
    const nextQuestions = questions.map((el) => {
  ...
  const handleIsRequiredChange: QuestionEditableProps["onRequiredChange"] = (
    event,
    question
  ) => {
    const nextQuestions = questions.map((el) => {
  ...
  const handleOptionChange: QuestionEditableProps["onOptionChange"] = (
    event,
    question,
    option
  ) => {
  ...
  const handleOptionDelete: QuestionEditableProps["onOptionDelete"] = (
    event,
    question,
    deletedOption
  ) => {
    const nextQuestions = questions.map((el) => {
      if (el.questionId === question.questionId) {
        return {
          ...el,
          optionList: el.optionList?.filter(
            (opt) => opt.optionId !== deletedOption.optionId
          ),
        };
      }
      return el;
  ...
  const handleOptionDragEnd: QuestionEditableProps["onOptionDragEnd"] = (
    result,
    question,
    options
  ) => {
  ...
  const handleOptionCreate: QuestionEditableProps["onOptionCreate"] = (
    e,
    question,
    index
  ) => {
  ...
  const handleOptionCreateOther: QuestionEditableProps["onOptionCreateOther"] =
    (e, question) => {
      let createdOption: Option | undefined;
      const nextQuestions = questions.map((el) => {
        if (el.questionId === question.questionId && el.optionList) {
  ...
  const handleRatingStartChange: QuestionEditableProps["onRatingStartChange"] =
    (e, question) => {
      const nextQuestions = questions.map((el) => {
        if (el.questionId === question.questionId) {
          return {
  ...
  const handleRatingEndChange: QuestionEditableProps["onRatingEndChange"] = (
    e,
    question
  ) => {
    const nextQuestions = questions.map((el) => {
  ...
  const handleRatingStartInputChange: QuestionEditableProps["onRatingStartInputChange"] =
    (e, question) => {
      const nextQuestions = questions.map((el) => {
        if (el.questionId === question.questionId) {
          return {
  ...
  const handleRatingEndInputChange: QuestionEditableProps["onRatingEndInputChange"] =
    (e, question) => {
      const nextQuestions = questions.map((el) => {
        if (el.questionId === question.questionId) {
          return {
